...
function default_1(chanson) {
    ...
    var $contPDF = document.createElement('a');
    ...
    if (date) {
        ...
        dateSpan.innerHTML = 'ajouté le ' + moment_1["default"](date).format('LLL') + '. ';
...
// ready(function () {
var _a = function (raws) {
    var keywords = lodash_1["default"].sortBy(lodash_1["default"].map(raws, cleanData), function (f) {
        return f.title.toLocaleLowerCase();
    });
    return { files: lodash_1["default"].groupBy(keywords, function (f) {
            return lodash_1["default"].first(f.years);
        }), keywords: keywords };
    function cleanData(r) {
        var keywords = tokenize(r.title);
        var clean = {
            title: r.title,
            name: r.name,
            date: r.date,
        ...
        var datesExtracted = false;
        var evsCheck = false;
        lodash_1["default"].forEach(keywords, function (str, index) {
            if (!datesExtracted) {
                if (!isNaN(+str)) {
                    var num = +str;
                    if (num / 1000 >= 1) clean.years.push('' + num);else clean.month = '' + str;
                    return;
        ...
        lodash_1["default"].forEach(clean.years.concat([clean.month, clean.evs]), function (r) {
            if (!r) return;
            clean.title = clean.title.replace(r, '');
        });
        clean.title = clean.title.trim();
        clean.name = '(' + (clean.month ? clean.month + '-' : '') + clean.years.join(', ') + ') ' + clean.title + (clean.evs ? " [" + clean.evs.toLocaleUpperCase() + "]" : '');
        return clean;
    }
}(window.files || []),
    files = _a.files,
...
(function buildMenu(files, $navigation) {
    // console.log(files)
    lodash_1["default"].forEachRight(files, function (yearFiles, year) {
        var $li = document.createElement('li');
        $li.classList.add('mv2', 'b', 'underline');
        $li.innerHTML = '' + year;
        $navigation.appendChild($li);
        lodash_1["default"].forEach(yearFiles, function (file) {
            var $li = document.createElement('li');
...
(function buildChanson(files, $main) {
    lodash_1["default"].forEachRight(files, function (filesYear, year) {
        var $cont = document.createElement('div');
        ...
        lodash_1["default"].forEach(filesYear, function (file) {
            // console.log(chanson.title);
            $main.appendChild(chansonTemplate_1["default"](file));
        });
    });
})(files, $main);
(function search(files, $sidebar, $hamburger) {
    ...
    var $parents = {};
    var $toc = {};
    lodash_1["default"].forEach(files, function (c) {
        $parents[c.slug] = document.getElementById('a--' + c.slug);
        $toc[c.slug] = document.getElementById('nav--' + c.slug);
    });
    var inverted = {};
    lodash_1["default"].forEach(files, function (file) {
        lodash_1["default"].forEach(file.keywords, function (t) {
            ;
            (inverted[t] || (inverted[t] = [])).push(file.slug);
        });
    });
    var searchable = lodash_1["default"].map(inverted, function (value, key) {
    ...
    function filterDisplay() {
        ...
        var results;
        lodash_1["default"].forEach(tokens, function (t) {
            var filtered = lodash_1["default"].flatMap(lodash_1["default"].filter(searchable, function (s) {
                return lodash_1["default"].includes(s.token, t);
            }), 'match');
            ...
            results = lodash_1["default"].intersection(results, filtered);
        });
        if (results.length === 0) {
        ...
        lodash_1["default"].forEach(results, function (r) {
            displayable[r] = true;
        });
        lodash_1["default"].forEach(displayable, function (display, id) {
    ...
    $searchInput.addEventListener('keydown', lodash_1["default"].debounce(filterDisplay, 300));
...
function tokenize(input) {
    return lodash_1["default"].filter(lodash_1["default"].split(lodash_1["default"].deburr(input).toLocaleLowerCase(), /[^\w]+/), function (t) {
        return t.length >= 2;
    });
}
// console.log(pages)
